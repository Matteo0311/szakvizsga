...
const JatekosEletkor = () => {
    ...
    useEffect(() => {
        jatekosokBetoltese();
        const mentettLegjobb = localStorage.getItem('legjobPontszamEletkor');
        if (mentettLegjobb) {
    ...
    const jatekosokBetoltese = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Szűrjük ki azokat, akiknek van életkora
                const szurtData = data.filter(j => j.foci_jatekos_eletkor && j.foci_jatekos_eletkor > 0);
    ...
    const tippelés = (valasztottJatekos) => {
        ...
        const aktualisErtek = parseFloat(aktualisJatekos.foci_jatekos_eletkor);
        const kovetkezoErtek = parseFloat(kovetkezoJatekos.foci_jatekos_eletkor);
        
        // Ha az aktuális játékosra kattintottak, azt gondolják, hogy ő az idősebb
        // Ha a következő játékosra kattintottak, azt gondolják, hogy ő az idősebb
        let helyes = false;
        ...
        if (helyes) {
            ...
            setTimeout(() => {
                ...
                if (ujPontszam > legjobPontszam) {
                    ...
                    localStorage.setItem('legjobPontszamEletkor', ujPontszam.toString());
                }
                ...
                >
                    <div className="player-info">
                        ...
                        <div className="player-value">
                            <h3>Életkor</h3>
                            <p className="value">{formatErtek(aktualisJatekos.foci_jatekos_eletkor)} <span className="unit">év</span></p>
                        </div>
                    </div>
                    {!megmutat && !jatekVege && (
                        <div className="click-hint">Kattints, ha ez az idősebb!</div>
                    )}
                </div>
                ...
                >
                    <div className="player-info">
                        ...
                        <div className="player-value">
                            <h3>Életkor</h3>
                            {megmutat ? (
                                <p className="value revealed">{formatErtek(kovetkezoJatekos.foci_jatekos_eletkor)} <span className="unit">év</span></p>
                            ) : (
